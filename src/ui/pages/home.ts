...
export function homePage(servers: Server[], query: string, category: string): string {
  ...
  return `
<div class="mb-8">
  <h1 class="text-3xl font-bold mb-2">MCP Server Marketplace</h1>
  <p class="text-gray-400">Discover and share MCP servers for Claude Desktop, Cursor, and more.</p>
</div>
...
${servers.length === 0
  ? `<div class="text-center py-16 text-gray-500">
       ...
       <a href="/submit" class="text-brand hover:underline text-sm mt-2 inline-block">Submit the first one →</a>
     </div>`
...
<a href="/server/${escHtml(s.id)}" class="block bg-gray-900 border border-gray-800 rounded-xl p-5 hover:border-gray-600 transition group">
  <div class="flex items-start justify-between mb-3">
    ...
    <span class="text-gray-500 text-xs">${escHtml(String(s.download_count))} installs</span>
  </div>
